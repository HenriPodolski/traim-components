...
const TraimAccordion = class {
    ...
    render() {
        ...
        return (index.h("div", { class: `accordion ${useCounterClass}` }, index.h("slot", null)));
    }
...
const TraimAccordionPane = class {
    ...
    render() {
        ...
        return [
            index.h("button", { role: "heading", "aria-expanded": this._isOpen.toString(), tabindex: !this.empty ? 0 : -1, class: `control ${isOpenClass} ${isEmptyClass}`, onClick: () => !this.empty && this.toggle(), innerHTML: this.header }),
            index.h("section", { "aria-hidden": !this._isOpen, class: `pane` }, index.h("slot", null)),
        ];
    }
};
...
const TraimAutocomplete = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.onSelect = index.createEvent(this, "selectAutocomplete", 7);
        this.onSearch = index.createEvent(this, "searchAutocomplete", 7);
        this.items = [];
    ...
    render() {
        return (index.h("div", { class: "autocomplete" }, index.h("input", { id: this.uid, name: this.uid, type: "search", class: "autocomplete__input", placeholder: this.placeholder, autocomplete: "off", value: this.value, onInput: (e) => this.search(e), onFocus: () => this.open(), onClick: () => this.open() }), (this._isOpen) && (index.h("div", { role: "menu", class: "autocomplete__list" }, this.items.map((item) => {
            const isActiveClass = this.activeItem === item ? 'is-active' : '';
            return (index.h("button", { role: "menuitem", class: `autocomplete__list-item ${isActiveClass}`, onClick: () => this.select(item) }, item.value.title));
        }), this.emptyMessage && this.items.length === 0 && (index.h("div", { role: "menuitem", "aria-disabled": true, class: "autocomplete__empty" }, this.emptyMessage))))));
    }
...
const TraimSlider = class {
    ...
    render() {
        ...
        return (index.h(index.Host, null, index.h("div", { class: `slider ${animationClass}` }, index.h("section", { ref: (el) => this.slidesWrapperElement = el, class: `slider__slides` }, index.h("slot", null)), this.countSlides > 1 && this.controls && (index.h("button", { onClick: () => this.gotoSlide(this.activeSlideIndex - 1), role: "button", class: `slider__button is-left`, "aria-label": this.controlLeftText }, index.h("span", { "aria-hidden": "true", class: `slider__button-inner` }, "\u276E"))), this.countSlides > 1 && this.controls && (index.h("button", { onClick: () => this.gotoSlide(this.activeSlideIndex + 1), role: "button", class: `slider__button is-right`, "aria-label": this.controlRightText }, index.h("span", { "aria-hidden": "true", class: `slider__button-inner` }, "\u276F"))))));
    }
    static get watchers() { return {
...
const TraimSliderSlide = class {
    ...
    render() {
        ...
        return (index.h(index.Host, null, index.h("div", { class: `slider-slide ${activeClass}` }, index.h("slot", null))));
    }
};
